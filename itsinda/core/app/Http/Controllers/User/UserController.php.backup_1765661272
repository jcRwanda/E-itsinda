...
class UserController extends Controller {
    public function home() {
        ...
        // USSD Contributions - fetch from API
        $widget['ussd_contributions'] = 0;
        $widget['ussd_amount'] = 0;
        $widget['ussd_pending'] = 0;
        $widget['ussd_pending_amount'] = 0;
        $widget['ussd_status'] = 'N/A';
        $widget['ussd_transactions'] = [];
        
        try {
            $response = \Illuminate\Support\Facades\Http::post('https://itsinda.garatech.rw/api/ussd/contributions/get', [
                'phone' => $user->mobile
            ]);
            ...
            if ($response->successful() && $response->json('status') === 'success') {
                ...
                $pending = collect($contributions)->where('status', 'PENDING');
                $completed = collect($contributions)->where('status', 'SUCCESS');
                
                $widget['ussd_contributions'] = $completed->count();
                $widget['ussd_amount'] = $data['balance'] ?? 0;
                $widget['ussd_pending'] = $pending->count();
                $widget['ussd_pending_amount'] = $pending->sum('amount');
                $widget['ussd_status'] = $pending->count() > 0 ? 'Pending' : 'Active';
                
                // Get last 5 transactions for display
                $widget['ussd_transactions'] = collect($contributions)
            ...
            \Illuminate\Support\Facades\Log::error('USSD API Error: ' . $e->getMessage());
        }
        ...
        return response()
            ->view('Template::user.dashboard', compact('pageTitle', 'user', 'credits', 'debits', 'widget'))
